...
class Boss(pygame.sprite.Sprite):
    ...
    def update(self):  # moving logic
        ...
        if self.HP == 25:
            self.current_image = pygame.transform.scale(pygame.image.load("assets/buffRat.png"), (200, 200))
            self.image_up = pygame.transform.scale(self.current_image, (200, 200))
            self.image_down = pygame.transform.scale(self.current_image, (200, 190))

        if (pygame.time.get_ticks() % 2000 > 1000):
            self.image_up = pygame.transform.scale(self.image_up, (200, 200))
            self.image = self.image_up
            self.rect.bottom = 350
        else:
            self.image_down = pygame.transform.scale(self.image_down, (200, 190))
        ...
        if self.moved >= 500:
            ...
            self.image_up = pygame.transform.flip(self.image_up, True, False)
            self.image_down = pygame.transform.flip(self.image_down, True, False)
        elif self.moved <= 0:
            self.move_right = True
            self.image_up = pygame.transform.flip(self.image_up, True, False)
            self.image_down = pygame.transform.flip(self.image_down, True, False)
